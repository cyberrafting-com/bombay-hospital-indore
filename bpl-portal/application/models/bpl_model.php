...
class Bpl_model extends CI_Model
{
    ...
    public function bpl_registrations($offset = '', $per_page = '') {
    ...
    public function get_bpl_registration_record($special_id='')
    {
        $this->db->where_in('admit_status',array(0,1));
        $this->db->where('specialty_id',$special_id);
        $this->db->from('bpl_registration');
        $query = $this->db->get();  
        if($query->num_rows()>0){ 
            return $query->result();
