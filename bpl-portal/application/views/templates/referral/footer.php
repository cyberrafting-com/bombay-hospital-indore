  ...
  <script type="text/javascript">
      jQuery(document).ready(function($) {
         $('#submit_btn').on('click',function(event) {
             $('#report_form').attr('action', '<?php echo base_url('referral/reports/index/') ?>');
             $('#report_form').submit();
        });
        
        $('#report_btn').on('click',function(event) {
           $('#report_form').attr('action', '<?php echo base_url('referral/reports/export/') ?>');
            $('#report_form').submit();
        });      
      });
    </script>
    ...
    <script type="text/javascript">
     ...
     $(document).ready(function(){
        ...
        $('#referral_name').on('change',function(){
          var member_id = $(this).val();
          var referral_centre  =  $('#referral_centre').val();
          if(member_id!='' &&  referral_centre!=''){
              $.post(BASEURL+'referral/bpl/get_member_detail/',{member_id:member_id,hospital_id:referral_centre},function(data){
                  var resp  = $.parseJSON(data);
                  if(resp.status==1){
                    $('#mobile_num').val(resp.mobile_no);
                    $('#designation').val(resp.designation);
                  }else{
                    $('#mobile_num').val('');
                    $('#designation').val('');
                  }
        ...
        $(".select2").select2();
        $('#date_of_referral').datepicker({ dateFormat:'dd/mm/yy',autoclose: true});
